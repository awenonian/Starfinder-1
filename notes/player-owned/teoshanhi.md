# Teoshanhi — "Shan"

**Pahtra (Meyel's Chosen) Operative (Ghost) 1** · Detective background
Player-owned. The GM does not change anything here without permission, except
rules-prescribed bookkeeping like hit point loss.

Built in session zero. Nothing on this sheet was rolled — all of it was chosen.

---

## Attributes

| Str | Dex | Con | Int | Wis | Cha |
|---|---|---|---|---|---|
| +0 | **+4** | +0 | +2 | +2 | +1 |

Derivation: Pahtra ancestry (Cha +1, Dex +1, free→Int +1; **Con flaw −1**);
Detective background (Wis +1, free→Dex +1); Operative class key attribute
(Dex +1); four free 1st-level boosts (Dex, Con, Int, Wis).

## Core statistics

- **Hit Points:** 16 (8 ancestry + 8 class + 0 Con)
- **Armor Class:** 18 (10 + 4 Dex + 3 trained + 1 Second Skin)
- **Speed:** 25 feet
- **Size:** Medium · **Senses:** Darkvision (black and white in darkness)
- **Class DC:** 17

| Defence | Rank | Modifier |
|---|---|---|
| Perception | Expert | **+7** |
| Fortitude | Trained | +3 |
| Reflex | Expert | **+9** |
| Will | Expert | +7 |

## Proficiencies

- **Attacks:** Expert simple & martial guns; trained advanced guns, all other
  weapons, unarmed. ("Gun" = any ranged weapon with the analog or tech trait.)
- **Defences:** Trained light armor & unarmored defense.

## Skills

Trained in all of the below (proficiency +3 at 1st level).

| Skill | Attr | Mod | Source |
|---|---|---|---|
| Acrobatics | Dex | +6 | class |
| Deception | Cha | +4 | Ghost specialization |
| Intimidation | Cha | +4 | class |
| Society | Int | +5 | background |
| Stealth | Dex | +6 | class |
| Thievery | Dex | +6 | class |
| Underworld Lore | Int | +5 | background |
| Piloting | Dex | +6 | class |

Dex-based skills include the −1 check penalty from Second Skin.

**Languages:** Common, Pahtra, Vesk, **Castrovelian, Trinary**.

## Class features

**Sharpshooter — Aim** (◆, Concentrate, Operative): designate one creature
within your first range increment as your **mark**. Your ranged Strikes against
your mark deal **+1d4 precision damage** and reduce its cover bonus by 1. One
mark at a time.

**Ghost specialization exploit:** you can **Aim at a hidden or undetected target
as a free action.** Pairs with darkvision — she is at her best working in the
dark against people who don't know where she is.

## Feats

- **Ancestry — Harmonic Sensitivity:** trained in Performance (or Assurance
  (Performance) if already trained). Can accurately recall any melody or lyrics
  heard within the last week; **+2 circumstance bonus to Deception and
  Performance checks involving mimicking speech or a specific sound.**
- **Operative — Mobile Aim** (Operative, Traversal): Stride and then Aim with a
  gun you're wielding.
- **Skill (Ghost) — Lie to Me:** use Deception to detect lies.
- **Skill (background) — Streetwise:** use your Society modifier to Gather
  Information; in your home settlement, use Society to Recall Knowledge
  (higher DC, no time cost).

## Equipment — 150 credits, 19 remaining

| Item | Price | Bulk | Notes |
|---|---|---|---|
| Rotating pistol | 30 | L | 1d6 **piercing**, range 60 ft, reload 1, magazine 6, 1 hand. **Agile, analog, concealable, concussive.** Projectile group. |
| Second Skin | 20 | L | +1 AC, Dex cap 4, check −1. Worn under clothing. |
| Infiltrator's toolkit, commercial | 30 | L | Required to Pick a Lock or Disable a Device. |
| Holoskin, commercial | 20 | L | Required to Impersonate someone. Doesn't change voice, scent, or mannerisms. |
| Comm unit | 7 | L | |
| Microphone | 8 | L | |
| Musical instrument, handheld | 8 | 1 | |
| Sunshades | 2 | — | |
| Data chip ×3 | 6 | — | |

**Attack — rotating pistol:** +9 to hit, 1d6 piercing. Agile, so the multiple
attack penalty is −4 / −8. Against her mark, +1d4 precision.

Carried Bulk ≈ 1.6 of 5.

## Planned

**Eyeswide Detective Dedication** at 2nd level (Feat 2, Uncommon, Archetype).
Prerequisites: **Wisdom +2** ✓ **and trained in Society** ✓. **Access: member of
the Eyeswide Agency** — a world requirement, not a build one.

Grants limited telepathy (30 ft), expert in Society, and the Streetwise skill
feat. **She already has Streetwise**, so the alternate benefit applies instead:
when she uses Society to Gather Information, a **critical failure becomes a
failure**. She can come back empty-handed. She can't make things worse by asking.

## Notes

**She is not trained in Diplomacy.** This was a deliberate choice, not an
oversight. Streetwise routes Gather Information through Society (+5), so her
information-gathering is covered; what she gives up is Make an Impression and
Request. Shan can lie to you, and she can lean on you, and she can find out
about you. She cannot win you over. Play her that way.

Ammunition purchasing has not been priced out; flagged if it ever matters.
